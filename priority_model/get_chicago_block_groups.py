...
def filter_chicago_block_groups(chicago_bounds_path, block_group_path, save_wkt= False):
    ...
    df_chicago_boundaries["geometry"] = df_chicago_boundaries["the_geom"].apply(wkt.loads)
    gdf_chicago = gpd.GeoDataFrame(df_chicago_boundaries, geometry="geometry", crs="EPSG:4326")
...
def plot_block_groups(cbg, save_image = None):
    ...
    plt.axis('off')
    if save_image:
        plt.savefig(config.MAPS_OUT + "Chicago_Block_Groups.png")
    plt.show()
...
def main():
    ...
    plot_block_groups(cbg, config.MAPS_OUT + "chicago_block_groups.png")
