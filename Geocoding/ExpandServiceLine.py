...
def ExpandAddresses(row):
    ...
    if len(address_parts) >= 3 and address_parts[0].isdigit() and address_parts[1].isdigit():
        ...
        if end_num > start_num:
            ...
            expandable.append(row['Address'])  
            expanded_rows= []
            for i in range(start_num, end_num+1, 2):
                new_row = row.copy()
                new_row['Address'] = f"{i} {base_address}"
        ...
        # iterate all the addresses. We will convert 57 to 6357
            ...
            expandable.append(row["Address"])
            expanded_rows= []
            for i in range(start_num, end+1, 2):
                new_row = row.copy()
                new_row['Address'] = f"{i} {base_address}"
                expanded_rows.append(new_row)
            #print(address, exp)
        ...
        # These are addresses where the end num is 0. From my research this appears to represent a block but I do not know how to expand this
        else:
            #Add address to unexpandable address list
            unexpandable.append(row["Address"])
            print(row["Address"])
            endzero.append(row["Address"])
            return [row]
...
expanded_df.to_csv("./ExpandedAddressBatches/all_service_line_info.csv", index=False)
